...
-- =============== TOOL SCANNER & UPGRADER (AI TOOLS) ===============
makeLabel(secAI, "━━━━━━ ⚒️ TOOL SCANNER & UPGRADER ━━━━━━")

local toolModState = {
    tools = {},           -- {tool, name, damage, tier, parent}
    targetDamage = 999,
    targetTier = 3,
}

-- Fungsi baca Damage dari tool (bisa jadi atribut / NumberValue)
local function getToolDamage(tool)
    -- 1. Cek atribut "Damage"
    local attr = tool:GetAttribute("Damage")
    if attr and type(attr) == "number" then return attr end
    -- 2. Cek NumberValue bernama "Damage"
    local val = tool:FindFirstChild("Damage")
    if val and val:IsA("NumberValue") then return val.Value end
    -- 3. Cek di Handle
    local handle = tool:FindFirstChild("Handle")
    if handle then
        attr = handle:GetAttribute("Damage")
        if attr and type(attr) == "number" then return attr end
        val = handle:FindFirstChild("Damage")
        if val and val:IsA("NumberValue") then return val.Value end
    end
    return "?"
end

-- Fungsi baca Tier dari tool
local function getToolTier(tool)
    local tierNames = {"Tier", "ToolTier", "Rarity", "Level"}
    for _, tn in ipairs(tierNames) do
        local attr = tool:GetAttribute(tn)
        if attr then return attr end
    end
    local val = tool:FindFirstChild("Tier") or tool:FindFirstChild("ToolTier")
    if val and val:IsA("NumberValue") then return val.Value end
    return "?"
end

-- Fungsi set Damage
local function setToolDamage(tool, newDamage)
    -- Cek atribut
    if tool:GetAttribute("Damage") ~= nil then
        tool:SetAttribute("Damage", newDamage)
        return true
    end
    local val = tool:FindFirstChild("Damage")
    ...
    local handle = tool:FindFirstChild("Handle")
    if handle then
        if handle:GetAttribute("Damage") ~= nil then
            handle:SetAttribute("Damage", newDamage)
            return true
        end
        val = handle:FindFirstChild("Damage")
        if val and val:IsA("NumberValue") then
...
-- Fungsi set Tier
local function setToolTier(tool, newTier)
    local tierNames = {"Tier", "ToolTier", "Rarity", "Level"}
    for _, tn in ipairs(tierNames) do
        if tool:GetAttribute(tn) ~= nil then
            tool:SetAttribute(tn, newTier)
            return true
        end
    end
    local val = tool:FindFirstChild("Tier") or tool:FindFirstChild("ToolTier")
...
local function scanAllTools()
    local found = {}
    local function scanContainer(container, parentName)
        for _, obj in ipairs(container:GetChildren()) do
            if obj:IsA("Tool") then
                table.insert(found, {
                    tool = obj,
                    name = obj.Name,
                    damage = getToolDamage(obj),
                    tier = getToolTier(obj),
                    parent = parentName,
                })
            end
        end
    ...
    if player.Backpack then scanContainer(player.Backpack, "Backpack") end
    if character then scanContainer(character, "Character") end
...
-- UI Elements
local toolStatusLabel = makeLabel(secAI, "Status: Idle")
toolStatusLabel.TextColor3 = Color3.fromRGB(160, 200, 255)

makeSlider(secAI, "Target Damage", 10, 9999, toolModState.targetDamage, function(v)
    toolModState.targetDamage = v
end)

makeSlider(secAI, "Target Tier", 1, 5, toolModState.targetTier, function(v)
    toolModState.targetTier = v
end)

-- Panel daftar tool
local toolPanel = Instance.new("ScrollingFrame", secAI)
toolPanel.Size = UDim2.new(1, 0, 0, 160)
toolPanel.BackgroundColor3 = Color3.fromRGB(24, 24, 30)
toolPanel.BorderSizePixel = 0
toolPanel.ScrollBarThickness = 5
toolPanel.ScrollBarImageColor3 = Color3.fromRGB(100, 100, 130)
toolPanel.CanvasSize = UDim2.new(0, 0, 0, 0)
Instance.new("UICorner", toolPanel).CornerRadius = UDim.new(0, 6)

local toolLayout = Instance.new("UIListLayout", toolPanel)
toolLayout.Padding = UDim.new(0, 3)
toolLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
    toolPanel.CanvasSize = UDim2.new(0, 0, 0, toolLayout.AbsoluteContentSize.Y + 8)
end)

-- Isi panel
local function fillToolPanel()
    for _, child in ipairs(toolPanel:GetChildren()) do
        if child:IsA("Frame") then child:Destroy() end
    end

    if #toolModState.tools == 0 then
        local lbl = Instance.new("TextLabel", toolPanel)
        lbl.Size = UDim2.new(1, 0, 0, 28)
        lbl.BackgroundTransparency = 1
        lbl.TextColor3 = Color3.fromRGB(180, 80, 80)
        lbl.Font = Enum.Font.Gotham
        lbl.TextSize = 12
        lbl.Text = "  Tidak ada tool ditemukan"
        return
    end

    for i, data in ipairs(toolModState.tools) do
        local row = Instance.new("Frame", toolPanel)
        row.Size = UDim2.new(1, -6, 0, 34)
        row.BackgroundColor3 = Color3.fromRGB(34, 34, 44)
        row.BorderSizePixel = 0
        Instance.new("UICorner", row).CornerRadius = UDim.new(0, 4)

        local info = string.format("[%s] %s | Dmg: %s | Tier: %s",
            data.parent, data.name, tostring(data.damage), tostring(data.tier))
        local infoLbl = Instance.new("TextLabel", row)
        ...
        local upgradeBtn = Instance.new("TextButton", row)
        upgradeBtn.Size = UDim2.new(0.35, 0, 0.8, 0)
        upgradeBtn.Position = UDim2.new(0.65, 0, 0.1, 0)
        upgradeBtn.Text = "⬆ Set"
        upgradeBtn.BackgroundColor3 = Color3.fromRGB(200, 140, 40)
        ...
        upgradeBtn.MouseButton1Click:Connect(function()
            local okDmg = setToolDamage(data.tool, toolModState.targetDamage)
            local okTier = setToolTier(data.tool, toolModState.targetTier)
            if okDmg or okTier then
                upgradeBtn.BackgroundColor3 = Color3.fromRGB(40, 180, 40)
                task.delay(1.2, function()
                    upgradeBtn.BackgroundColor3 = Color3.fromRGB(200, 140, 40)
                end)
            else
                upgradeBtn.BackgroundColor3 = Color3.fromRGB(180, 40, 40)
                task.delay(1.2, function()
                    upgradeBtn.BackgroundColor3 = Color3.fromRGB(200, 140, 40)
                end)
            end
...
-- Tombol Scan
local scanToolBtn = makeButton(secAI, "🔍 Scan Tools", Color3.fromRGB(60, 130, 200))
scanToolBtn.MouseButton1Click:Connect(function()
    toolStatusLabel.Text = "⏳ Scanning..."
    toolModState.tools = scanAllTools()
    fillToolPanel()
    toolStatusLabel.Text = string.format("✅ %d tool ditemukan", #toolModState.tools)
end)

-- Tombol Upgrade All in Backpack
local upgradeAllBackpackBtn = makeStyledButton(secAI, "⚡ Upgrade All Backpack", Color3.fromRGB(220, 120, 20))
upgradeAllBackpackBtn.MouseButton1Click:Connect(function()
    local count = 0
    for _, data in ipairs(toolModState.tools) do
        if data.parent == "Backpack" then
            setToolDamage(data.tool, toolModState.targetDamage)
            setToolTier(data.tool, toolModState.targetTier)
            count = count + 1
        end
    end
    toolStatusLabel.Text = string.format("⚡ %d tools di-upgrade", count)
end)

-- Tombol Upgrade Equipped
local upgradeEquippedBtn = makeButton(secAI, "🛠 Upgrade Equipped", Color3.fromRGB(200, 180, 40))
upgradeEquippedBtn.MouseButton1Click:Connect(function()
    local tool = character and character:FindFirstChildOfClass("Tool")
    if not tool then
        toolStatusLabel.Text = "❌ Tidak ada tool yang di-equip"
        return
    end
    setToolDamage(tool, toolModState.targetDamage)
    setToolTier(tool, toolModState.targetTier)
    toolStatusLabel.Text = "✅ Tool " .. tool.Name .. " di-upgrade"
end)

makeLabel(secAI, "💡 Tips: Beberapa game memvalidasi damage di server,\n   jadi perubahan hanya terlihat di client (visual/nilai).")
