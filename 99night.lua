...
-- =============== AI REVERSE ENGINEERING (AI TOOLS) ===============
makeLabel(secAI, "━━━━━━ 🔍 AI REVERSE ENGINEERING ━━━━━━")
...
-- Fungsi analisis sederhana (simulasi AI)
local function analyzeCode(code, remoteObj)
    local vulns = {}
    local report = ""

    -- 1. Cari pengecekan boolean flag
    if code:find("InteractedWith") or code:find("CanUse") or code:find("IsActivated") then
        table.insert(vulns, "🌐 Boolean flag (InteractedWith/CanUse) – bisa di‑spam race condition")
    end

    -- 2. Cek perbandingan Owner/LastOwner
    if code:find("Owner") and not code:find("LastOwner") then
        table.insert(vulns, "👤 Hanya cek Owner, tidak ada validasi LastOwner – mungkin bisa di‑curi")
    end

    -- 3. Cek validasi atribut BurnFuel/Bahan Bakar tanpa batas
    if code:find("BurnFuel") and not code:find("math%.max") and not code:find("math%.min") then
        table.insert(vulns, "⛽ BurnFuel tanpa batas – bisa dimanipulasi client")
    end

    -- 4. Remote tanpa parameter keamanan
    if remoteObj then
        if remoteObj:IsA("RemoteEvent") then
            report = report .. "📡 RemoteEvent terdeteksi: " .. remoteObj:GetFullName() .. "\n"
        elseif remoteObj:IsA("RemoteFunction") then
            report = report .. "📡 RemoteFunction terdeteksi: " .. remoteObj:GetFullName() .. "\n"
        end
        report = report .. "🔹 Parameter: " .. table.concat(remoteObj:GetAttribute("Parameters") or {}, ", ") .. "\n"
    end

    -- 5. Cek apakah ada pengecekan client-side yang tidak aman
    if code:find("client") or code:find("LocalPlayer") then
        table.insert(vulns, "💻 Kode client‑side ditemukan – server mungkin tidak memvalidasi ulang")
    end

    -- Bangun laporan
    if #vulns == 0 then
        report = report .. "✅ Tidak ditemukan celah mencurigakan (analisis AI terbatas)\n"
    else
        report = report .. "⚠️ Potensi celah:\n"
        for _, v in ipairs(vulns) do
            report = report .. "• " .. v .. "\n"
        end
    end

    reverseState.analysisResult = report
    reverseState.vulnerabilities = vulns
    return report
end

-- Generate exploit template
local function generateExploit(remoteObj, targetObjectName)
    if not remoteObj then return "-- Remote tidak ditemukan" end
    local remotePath = remoteObj:GetFullName()
    local exploit = [[
-- Auto‑Generated Exploit by AI Reverse Engineer
local remote = game:GetService("]] .. remoteObj:GetService() .. [[")]] .. 
    (remoteObj:IsA("RemoteEvent") and "[RemoteEvent]" or "[RemoteFunction]") .. 
    [[.]] .. remotePath .. [[
local targetName = "]] .. (targetObjectName or "Fuel Canister") .. [["

-- Cari semua target
for _, obj in ipairs(workspace:GetDescendants()) do
    if obj:IsA("Model") and obj.Name == targetName then
        -- Spam sebelum flag di‑set server
        for i = 1, 10 do
            task.spawn(function()
                pcall(function()
...
print("Exploit executed on " .. targetName)
]]
    return exploit
end

-- UI Elements
local reverseInputLabel = makeLabel(secAI, "📥 Masukkan kode atau RemoteEvent path:")
local reverseInputBox = Instance.new("TextBox", secAI)
reverseInputBox.Size = UDim2.new(1, 0, 0, 80)
reverseInputBox.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
reverseInputBox.TextColor3 = Color3.new(1, 1, 1)
reverseInputBox.Font = Enum.Font.Code
reverseInputBox.TextSize = 13
reverseInputBox.TextXAlignment = Enum.TextXAlignment.Left
reverseInputBox.TextYAlignment = Enum.TextYAlignment.Top
reverseInputBox.MultiLine = true
reverseInputBox.PlaceholderText = "Tempel kode di sini..."
reverseInputBox.Text = ""
reverseInputBox.BorderSizePixel = 0
Instance.new("UICorner", reverseInputBox).CornerRadius = UDim.new(0, 6)

-- Tombol Auto-Scan Remote
local scanRemoteBtn = makeButton(secAI, "📡 Scan Remote dari Path", Color3.fromRGB(100, 120, 200))
scanRemoteBtn.MouseButton1Click:Connect(function()
    local path = reverseInputBox.Text
    local obj
    pcall(function()
        obj = loadstring("return " .. path)()
    end)
    if obj and (obj:IsA("RemoteEvent") or obj:IsA("RemoteFunction")) then
        reverseState.targetRemote = obj
        reverseState.inputCode = path .. " (Remote ditemukan)"
        reverseInputBox.Text = reverseState.inputCode
        analyzeCode("", obj)
        reverseOutputBox.Text = "🔍 Analisis Remote:\n" .. reverseState.analysisResult
    else
        reverseInputBox.Text = "❌ Remote tidak valid: " .. path
    end
end)

-- Tombol Analisis
local analyzeBtn = makeStyledButton(secAI, "🧠 Analisis Kode", Color3.fromRGB(160, 80, 200))
analyzeBtn.MouseButton1Click:Connect(function()
    local code = reverseInputBox.Text
    reverseState.inputCode = code
    local remote = reverseState.targetRemote
    local report = analyzeCode(code, remote)
    reverseOutputBox.Text = "📊 Hasil Analisis AI:\n" .. report
end)

-- Tombol Generate Exploit
local generateExploitBtn = makeButton(secAI, "⚡ Generate Exploit", Color3.fromRGB(200, 100, 50))
generateExploitBtn.MouseButton1Click:Connect(function()
    if not reverseState.targetRemote and reverseState.inputCode == "" then
        reverseOutputBox.Text = "❌ Butuh RemoteEvent atau kode untuk generate exploit"
        return
    end
    local remote = reverseState.targetRemote
    -- Jika tidak ada remote dari scan, coba ambil dari input path
    if not remote then
        pcall(function()
            local path = reverseInputBox.Text:match("(.-)\n") or reverseInputBox.Text
            remote = loadstring("return " .. path)()
        end)
    end
    if not remote then
        reverseOutputBox.Text = "❌ RemoteEvent/RemoteFunction tidak ditemukan"
        return
    end
    -- Ambil objek target dari analisis (Fuel Canister, dsb)
    local targetName = "Fuel Canister" -- default
    if reverseState.vulnerabilities[1] then
        targetName = "Fuel Canister" -- bisa disesuaikan
    end
    local exploit = generateExploit(remote, targetName)
    reverseOutputBox.Text = "✅ Exploit generated:\n" .. exploit
end)

-- Output box
local reverseOutputLabel = makeLabel(secAI, "📊 Output Analisis:")
local reverseOutputBox = Instance.new("TextBox", secAI)
reverseOutputBox.Size = UDim2.new(1, 0, 0, 140)
reverseOutputBox.BackgroundColor3 = Color3.fromRGB(20, 20, 25)
reverseOutputBox.TextColor3 = Color3.new(0.8, 1, 0.8)
reverseOutputBox.Font = Enum.Font.Code
reverseOutputBox.TextSize = 12
reverseOutputBox.TextXAlignment = Enum.TextXAlignment.Left
reverseOutputBox.TextYAlignment = Enum.TextYAlignment.Top
reverseOutputBox.MultiLine = true
reverseOutputBox.Text = "Hasil analisis akan muncul di sini..."
reverseOutputBox.BorderSizePixel = 0
reverseOutputBox.ClearTextOnFocus = false
Instance.new("UICorner", reverseOutputBox).CornerRadius = UDim.new(0, 6)

-- Tombol Clear
local clearReverseBtn = makeButton(secAI, "🗑️ Clear", Color3.fromRGB(100, 100, 100))
clearReverseBtn.MouseButton1Click:Connect(function()
    reverseInputBox.Text = ""
    reverseOutputBox.Text = ""
    reverseState.inputCode = ""
    reverseState.targetRemote = nil
    reverseState.analysisResult = ""
    reverseState.vulnerabilities = {}
end)
